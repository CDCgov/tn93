...
def get_nucleotide_frequency(pairwise_counts):
...
def get_counts(seq1, seq2, match_mode):
...
def get_counts_resolve(seq1, seq2):
...
def get_counts_average(seq1, seq2):
    map_character, resolutions, resolutionsCount = get_constants()
    length = min(len(seq1), len(seq2))
    pairwise_counts = [
            # A, C, G, T
...
def get_counts_gapmm(seq1, seq2):
    map_character, resolutions, resolutionsCount = get_constants()
    length = min(len(seq1), len(seq2))
    pairwise_counts = [
            # A, C, G, T
